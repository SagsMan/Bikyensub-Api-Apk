 ...
 * Deploy to: api.bikyensub.com.ng/api.php
 * Usage:     https://api.bikyensub.com.ng/api.php?action=XXX
...
case 'health':
case 'ping':
    api_response(['message' => 'Bikyensub API is running', 'version' => '2.0', 'provider' => 'PaymentPoint', 'time' => date('Y-m-d H:i:s')]);
    break;
...
case 'profile':
    ...
    api_response([
        ...
        'referral_link'  => 'https://bikyensub.com.ng/easyfinder/dashboard/register?join_with_referal=' . ($user['referal_token'] ?? ''),
        'wallet_balance' => $bal,
        'has_account'    => !empty($accounts),
        'acc_no'         => $primary['account_number'] ?? '',
        'bank_name'      => $primary['bank_name'] ?? '',
...
case 'buy_airtime':
    ...
    if (file_exists(__DIR__ . '/fcm_helper.php')) {
        ...
        $pushMsg = $status
            ? "Airtime recharge of N{$amount} to {$number} was successful."
            : "Airtime recharge of N{$amount} to {$number} failed. Wallet refunded.";
        sendUserPushNotification($conn, $user['email'], 'Airtime Recharge', $pushMsg, ['type' => 'airtime']);
    }
...
case 'buy_data':
    ...
    if (file_exists(__DIR__ . '/fcm_helper.php')) {
        include_once __DIR__ . '/fcm_helper.php';
        $pushMsg = $status
            ? "Data purchase of N{$amount} to {$number} was successful."
            : "Data purchase of N{$amount} to {$number} failed. Wallet refunded.";
        sendUserPushNotification($conn, $user['email'], 'Data Purchase', $pushMsg, ['type' => 'data']);
    }
...
case 'get_referral_stats':
    ...
    api_response([
        ...
        'referral_link'  => 'https://bikyensub.com.ng/easyfinder/dashboard/register?join_with_referal=' . $refCode,
        'total_referred' => count($referred),
        'total_earnings' => $total,
        'referred_users' => $referred,
        'share_message'  => 'Join Bikyensub and earn on every data, airtime purchase! Use my referral code: ' . $refCode
                            . ' — Sign up at https://bikyensub.com.ng/easyfinder/dashboard/register?join_with_referal=' . $refCode,
    ]);
    break;
